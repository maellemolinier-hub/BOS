...
Pause courte (même jour ou lendemain) ou question spécifique, sans trigger organize, avec un focus actif dans `Core/Actions.md` : identifier le skill du focus en cours à partir du bottleneck le plus récent dans `Core/Diagnosis.md` et du focus actuel dans `Core/Actions.md` (quel que soit leur format exact — liste de problèmes par impact ou goulot unique nommé, selon que le dernier remplissage vient d'`onboard` ou d'un diagnostic complet), router dessus, ou vers l'exécution en mode par défaut si le focus ne nécessite pas de skill spécialisé. **Pas de diagnostic complet** — reprendre là où on en était.
